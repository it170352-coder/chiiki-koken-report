...
export default async function ProductsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-amber-900">商品管理</h1>

      <form
        action={createProduct}
        className="grid gap-3 rounded-2xl border border-amber-100 bg-white p-5 sm:grid-cols-[2fr_1fr_1fr_auto]"
      >
        <input
          name="name"
          required
        ...
        {list.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[480px] text-sm">
              <thead className="border-b border-amber-100 bg-amber-50/50 text-left text-gray-500">
                <tr>
