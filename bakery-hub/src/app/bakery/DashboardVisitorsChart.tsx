  ...
  return (
    <div className="rounded-2xl border border-bark-100 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link href="/bakery/visitors" className="text-xs font-medium text-bark-600 hover:underline">
          入力する →
        </Link>
      </div>
      ...
      {data.length === 0 ? (
        <div className="flex h-[160px] flex-col items-center justify-center gap-2 text-gray-400">
          ...
          <Link href="/bakery/visitors" className="text-xs text-bark-600 hover:underline">
            来客数を入力する
          </Link>
        </div>
      ) : (
