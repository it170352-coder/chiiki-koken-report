...
export default async function NewReservationPage() {
  ...
  const pickupHint =
    pickupStart && pickupEnd ? `受け渡し日時の目安：${pickupStart}〜${pickupEnd}` : "";
    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          />
          <p className="mt-1 text-xs text-gray-400">空白の場合は顧客未指定になります</p>
        </div>

        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">
            受け渡し日時 <span className="text-red-500">*</span>
          </label>
          <input
            type="datetime-local"
            name="pickup_at"
